...
#conflicted::conflict_prefer(name = 'as_adjacency_matrix', winner = 'igraph')
...
call_kstar_net <- function(matrix, sparsity, distFun, cores, k_star, knn) {
    MoNETA::k_star_net(matrix = matrix,
                   sparsity = sparsity,
                   distFun = distFun,
                   cores = cores,
                   knn = knn,
                                                  ...
                                                  shinydashboard::box(width=12,
                                                                      ...
                                                                      shinydashboard::tabBox(width=12,
                                                                                             ...
                                                                                             shiny::tabPanel(title = shiny::h3(shiny::span('Transition', style = "font-weight: bold")),
                                                                                                             ...
                                                                                                             shiny::fluidRow(
                                                                                                                 shiny::column(width = 12,
                                                                                                                               shiny::h4(shiny::span('Transition layer matrix', style = "font-weight: bold")),
...
server <- function(input, output, session) {
    ...
    output$info_box_1 <- renderText({
        HTML("<br/> <span style='font-weight:normal;'>
            ...
            The sample IDs are stored in the <b>Case</b> column.
    ...
    shiny::observeEvent(input$load_anno_button, {
        ...
        if (input$anno_example_opt == 'No'){
            ...
            data("pdata", package = 'MoNETA')
            listFiles[['annotation']] <- pdata
            annotation(listFiles)
        }else{
            return(NULL)
        }
    ...
    output$info_box_3 <- renderText({
        ...
        <p align='justify'>
            ...
            v) dynamic or fixed choice of the number of neighbors per node (<b>k_star</b>). <br/>
            *Euclidean and Cosine distances are recommended for continuous data,
            ...
            <p align='justify'>
            Once the network is created, the plot is shown. The nodes are colored according to their node ID by default,
    ...
    shiny::observeEvent(input$dr_btn, {
        if (length(shiny::reactiveValuesToList(RWR_output)) == 0){
            ...
            if (input$dr_method == 'UMAP'){
                ...
                dr_mat <-  MoNETA::get_tsne_embedding(matrix = emb_mat,
    ...
    output$omics_net_arguments <- shiny::renderUI({
        ...
        thetabs <- lapply(names(mats), function(x) {
            ...
            omics_network_list[[paste0('generate_omics_net_', x)]] <- shiny::observeEvent(input[[paste0('generate_omics_net_', x)]], {
                ...
                net <- call_kstar_net(matrix = mats[[x]],
                                          sparsity = isolate(input[[paste0('sparsity_', x)]]),
                                          distFun = isolate(input[[paste0('distFUN_', x)]]),
                                          knn = isolate(input[[paste0('knn_', x)]]),
                                          k_star = isolate(ifelse(input[[paste0('kstar_', x)]] == 'YES', TRUE, FALSE)),
                                          cores = isolate(input[[paste0('cores_', x)]]))
    ...
    output$info_box_6 <- renderText({
        HTML("<br/> <span style='font-weight:normal;'>
            ...
            which is a multi-layer network with as many layers as the number of networks inferred in the previous steps,
            all sharing the same set of nodes. </span> <br/>
            </p> <hr style='border-top: 1px solid white;'>
            ...
            <p align='justify'>
             ...
             network will be retained and will contribute to the node visit probability
             in the next phase of the pipeline. Subsequently,
             you can filter the multiplex network by selecting a threshold.
             In this case, only edge weights greater than the threshold will be retained. </span> <br/>
            </p> <hr style='border-top: 1px solid white;'>
    ...
    output$multi_net_sum_info <- shiny::renderUI({
        ...
        shiny::isolate({
            ...
            if (!is.null(multiplex)){
                ...
                n_layers <- multiplex %>%  dplyr::select(EdgeType) %>% distinct(.) %>% nrow(.)
    ...
    output$info_box_7 <- renderText({
        HTML("<br/> <span style='font-weight:normal;'>
            <p align='justify'>
            Welcome to the MoNETA Shiny app, an R package for network-based multi-omics data integration.<br/>
            Here, you can integrate multi-omics data through the Random Walk with Restart (RWR) algorithm,
            that simulates the traversal of an imaginary particle, called random walker, across the multiplex network. <br/>
            <hr style='border-top: 1px solid white;'>
            ...
            <p align='justify'>
                ...
                omics layers with probability proportional to the number of shared relationships beteween layers. <br/>
                &ensp; By default, the <b><span style='color:red;'>inter-layer</span></b> edges connect nodes representing the same entity across layers, but
    ...
    shiny::observeEvent(input$gen_multiplex_btn, {
        n_layers <- multiplex_network() %>%  dplyr::select(EdgeType) %>% distinct(.) %>% nrow(.)

        if (n_layers == 1){
            shinyjs::disable("delta")
            shinyjs::disable("bioInf_transition")
            shinyjs::disable("editable_trans_mat")
            shinyjs::disable("omics_jump_neigh")
        }else{
            shinyjs::enable("delta")
            shinyjs::enable("bioInf_transition")
            shinyjs::enable("editable_trans_mat")
            shinyjs::enable("omics_jump_neigh")
        }
    })
    ...
    output$info_box_8 <- renderText({
        HTML("<br/> <span style='font-weight:normal;'>
            ...
            <span style='font-weight:normal;'>
            <p align='justify'>
            The denoised and the dimensionality-reduced matrices can be downloaded from the <b>download box</b> located on the left-hand side,
    ...
    output$dr_opt <- shiny::renderUI({
        shinydashboard::box(width = 12, status = 'primary', solidHeader = FALSE, collapsible = FALSE,
                            ...
                            shiny::radioButtons(inputId = 'emb_opt',
                                                label = shiny::h4(shiny::span('Do you want to denoise the RWR similarity matrix via embedding?', style = "font-weight: bold")),
                                                choices = c('YES', 'NO'), selected = 'NO'),
                            shiny::conditionalPanel(
    ...
    data_orig_list <- shiny::reactive({
        if (length(shiny::reactiveValuesToList(dr_output)) != 0){
            ...
            data_shared <- SharedData$new(shared_input)
        ...
        shiny::isolate({
            if (length(shiny::reactiveValuesToList(dr_output)) == 0){
                ...
                observe({
                    ...
                    plotlyProxy(outputId = "dr_plot1", session) %>%
                ...
                return(shinydashboard::box(width = 12, height = 800,
                                           shinycssloaders::withSpinner(plotly::plotlyOutput('dr_plot1',)),
                                           shiny::hr(),
    ...
    output$data_orig_table <- renderDT({
        if (length(shiny::reactiveValuesToList(dr_output)) != 0) {
    ...
    output$info_box_9 <- renderText({
        HTML("<br/> <span style='font-weight:normal;'>
            <p align='justify'>
            Welcome to the MoNETA Shiny app, an R package for network-based multi-omics data integration.<br/>
            Here, you can conduct a clustering analysis on the dimensionality-reduced RWR
            similarity matrix. You can choose different clustering algorithms as well as add information
            present in the uploaded annotation file. </span> <br/>
            </p> <hr style='border-top: 1px solid white;'>
            ...
            <p align='justify'>
             ...
             decided to denoise the RWR similarity matrix, or from the dimensionality-reduced matrix
             via UMAP/PCA/tSNE. <br/>
             &ensp; If you select the <span style='color:steelblue4;'><b>kmeans</b></span>, you just hav  to
             set the number of desired clusters (<b>k</b>).<br/>
             &ensp; If you select the <span style='color:steelblue4;'><b>dbscan</b></span>, you just have to
             set the <b>&epsilon;</b> parameter.<br/>
             &ensp; If you select the <span style='color:steelblue4;'><b>hclust</b></span>, you just have to
            ...
            <b> Annotation </b>: <br/> <span style='font-weight:normal;'>
            <p align='justify'>
             The annotation file can be used to differentiate the shape of the points in the plot
             based on a specific column, or it can be used instead of a clustering
             algorithm to detect groups of points in the plot. </span> <br/>
            </p> <hr style='border-top: 1px solid white;'>

            <span style='font-weight:normal;'> <p align='justify'>
            The annotation table used to enrich the plot can be copied or downloaded in
            .csv and .xlsx format from the interactive table on the left, while
            the plot can be downloaded  by clicking on the <b>download plot</b> button or on the <i class='fas fa-camera'></i> icon
    ...
    output$custom_anno_selector <- shiny::renderUI({
        if (input$cluster_method == 'annotation'){
            ...
            output <- tagList()
            output[[1]] <-
                shiny::selectInput(inputId = 'custom_anno_selector',
                                   label = shiny::h4(shiny::span('Select a column to color data points',style = "font-weight: bold")),
                                   choices = colnames(anno_tab), selected = NULL)
            output[[2]] <-
                shiny::selectInput(inputId = 'custom_anno_shape_selector',
                                   label = shiny::h4(shiny::span('Select a column to shape data points',style = "font-weight: bold")),
                                   choices = colnames(anno_tab), selected = NULL)
            output
    ...
    output$cl_opt <- shiny::renderUI({
        ...
        output$shape_opt <- shiny::renderUI({
            if ( ( !is.null(anno()) |  !is.null(anno1()) ) & input$cluster_method %in% c('kmeans', 'dbscan', 'hclust') ){
                ...
                output[[1]] <-
                    shiny::hr()
                output[[2]] <-
                    shiny::radioButtons(inputId = 'shape_bool',
                                        label = shiny::h4(shiny::span('Do you want to use particular shape for points?',style = "font-weight: bold")),
                                        c('YES', 'NO'), selected = 'NO')
                output[[3]] <-
                    shiny::conditionalPanel(
                        condition = 'input.shape_bool == "YES"',
                        shiny::selectInput(inputId = 'select_shape',
                                           label = shiny::h4(shiny::span('Select a column to shape data points',style = "font-weight: bold")),
                                           choices = colnames(anno_table))
                    )
                output
        ...
        shinydashboard::box(width = 12, status = 'primary', solidHeader = FALSE,
                            ...
                            shiny::conditionalPanel(
                                condition = "input.cluster_method == 'hclust'",
                                shiny::sliderInput("h",
                                                   label = shiny::h4(shiny::span("Select h for cutree function:",style = "font-weight: bold")),
    ...
    output$cl_plot_box <- shiny::renderUI({
        ...
        shiny::isolate({
            if (is.null(shiny::reactiveValuesToList(clusters)$cluster)){
                ...
                return(shinydashboard::box(width = 12,
    ...
    output$cluster_table_box <- shiny::renderUI({
        if (!is.null(shiny::reactiveValuesToList(clusters)$cluster)){
            ...
            shinydashboard::box(width = 12, DT::DTOutput('cluster_table'))
        } else{
            return(NULL)
        }
    })

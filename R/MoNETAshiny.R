  ...
  shiny::observeEvent(input$cl_btn, {
    shiny::isolate(
      if (input$cluster_method == 'hclust' & !(is.null(shiny::reactiveValuesToList(clusters)$tree))){
        updateshiny::sliderInput(inputId = "h", max = round(base::attr(shiny::reactiveValuesToList(clusters)$tree, "height"), digits = 2)
                          )
      }else {
        updateshiny::sliderInput(inputId = "h", max = 100)
      }
    )
  })
